...
Orbit already has the runtime pieces needed for long-running and partially unsupervised agent work, but the guidance for how to operate, validate, and trust those workflows is scattered. This note captures:
...
These are useful as local coding guidance, but they are not a coherent unsupervised-development handbook. In practice, the CLI one is also stale enough to still refer to `orbit-cli`, so it should not be treated as the canonical source for current behavior.

## How unsupervised development currently works in Orbit

Based on the current docs and codebase structure, the operating model is roughly:

1. The user starts `orbit`, `orbit codex`, `orbit gemini`, or another wrapped agent flow.
...
Important consequence: Orbit already has a protocol-level understanding of subagents and tool-spawned sidechains. The missing layer is not transport. The missing layer is process: when unsupervised operation is allowed, how it is validated, and what counts as evidence that it is safe and working.

## What is missing

There is no single document that answers these operator questions:

- What counts as an approved unsupervised workflow in Orbit?
...
That means Orbit has the runtime for autonomous behavior, but not the operating manual.
...
The most important ideas worth copying into Orbit are:
...
This matches Orbit's needs well. Orbit wraps multiple agents already, so a portable procedural layer is more valuable than yet another provider-specific prompt convention.
...
This lines up with Orbit's existing protocol support for `subagent` event streams. The runtime already understands subagent identity; the missing piece is guidance for when and how to use that capability well.

## Implications for Orbit

The practical conclusion is:

- Orbit does **not** need to invent subagent transport first.
- Orbit does **not** primarily need more architecture docs.
- Orbit **does** need an operator/process layer for autonomous work.
...
3. `docs/skills-and-subagents.md`
   ...
   - how Orbit should interoperate with existing agent skill directories

## Recommended design direction

If Orbit decides to adopt skills more explicitly, the most compatible path is likely:
...
- keep Orbit focused on orchestration, observability, and validation rather than becoming a separate skill runtime

That approach keeps Orbit aligned with the broader ecosystem while preserving its real value: remote control, daemonized execution, protocol normalization, and session observability across providers.

## Bottom line

Orbit currently has the substrate for unsupervised development, but not the handbook.
