...
This document describes the Orbit CLI (`packages/orbit-cli`) and its daemon. The CLI is both an interactive tool and a background session manager that keeps machine state in sync with the server.
...
- **Persistence/config:** `src/persistence.ts` + `src/configuration.ts` manage local state in `~/.orbit`.
...
```mermaid
flowchart TD
    Start([orbit ...]) --> Parse[Parse subcommand]
...
graph LR
    ...
    subgraph "Environment Overrides"
        ...
        E3[ORBIT_WEBAPP_URL]
        E4[ORBIT_VARIANT]
        E5[ORBIT_EXPERIMENTAL]
        E6[ORBIT_DISABLE_CAFFEINATE]
    end
...
- `ORBIT_SERVER_URL` and `ORBIT_WEBAPP_URL` override defaults.
- `ORBIT_VARIANT`, `ORBIT_EXPERIMENTAL`, `ORBIT_DISABLE_CAFFEINATE` control behavior.
...
- CLI entry: `packages/orbit-cli/src/index.ts`
- Daemon: `packages/orbit-cli/src/daemon`
- Control server/client: `packages/orbit-cli/src/daemon/controlServer.ts`, `packages/orbit-cli/src/daemon/controlClient.ts`
- API clients: `packages/orbit-cli/src/api`
- Persistence: `packages/orbit-cli/src/persistence.ts`
- Config: `packages/orbit-cli/src/configuration.ts`
