...
This document describes the Orbit backend structure as implemented in `packages/orbit-server`. It focuses on how the server is wired, how data flows through the system, and which subsystems handle which responsibilities.
...
## Process lifecycle
Entry point: `packages/orbit-server/sources/main.ts`.
...
3. Initialize crypto modules:
   - `initEncrypt()` derives a KeyTree from `ORBIT_MASTER_SECRET`.
...
- Tokens are generated and verified by privacy-kit using `ORBIT_MASTER_SECRET`.
...
graph TB
    ...
    S1 & S2 & S3 & S4 --> |KeyTree from ORBIT_MASTER_SECRET| DB
...
- The server only encrypts/decrypts **service tokens** (GitHub OAuth tokens, vendor tokens) using the KeyTree derived from `ORBIT_MASTER_SECRET`.
...
- API server: `packages/orbit-server/sources/app/api/api.ts`
- Socket server: `packages/orbit-server/sources/app/api/socket.ts`
- Event routing: `packages/orbit-server/sources/app/events/eventRouter.ts`
- Presence: `packages/orbit-server/sources/app/presence`
- Storage: `packages/orbit-server/sources/storage`
- Prisma schema: `packages/orbit-server/prisma/schema.prisma`
