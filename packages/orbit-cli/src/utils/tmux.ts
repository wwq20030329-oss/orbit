 ...
 * Ensures consistent tmux handling across orbit-cli with proper session naming
 */
...
export class TmuxUtilities {
    ...
    public static readonly DEFAULT_SESSION_NAME = "orbit";
    ...
    async spawnInTmux(
        ...
        try {
            ...
            // - undefined: Use first existing session or create "orbit"
            // - empty string: Use first existing session or create "orbit"
            ...
            if (!sessionName) {
                ...
                if (listResult && listResult.returncode === 0 && listResult.stdout.trim()) {
                    ...
                    // No sessions exist, create "orbit"
                    sessionName = 'orbit';
                    logger.debug(`[TMUX] No existing sessions, using default: ${sessionName}`);
                }
            }

            const windowName = options.windowName || `orbit-${Date.now()}`;
