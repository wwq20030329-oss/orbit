 ...
 *     │                                         
 *     ├─► generateHookSettingsFile(port) ──► ~/.orbit/tmp/hooks/session-hook-<pid>.json
 ...
 * 
 * ### Triggered By
 * - `orbit` (fresh start) - new session created
 * - `orbit --continue` - continues last session (may fork)
 * - `orbit --resume` - interactive picker, then resume
 * - `orbit --resume <id>` - resume specific session
 * - `/compact` command - compacts and forks session
 * - Double-escape fork - user forks conversation in CLI
 * 
 * ### Why Not Use File Watching?
 * File watching has race conditions when multiple Orbit processes run.
 * With hooks, Claude directly tells THIS specific process about its session,
 * ensuring 1:1 mapping between Orbit process and Claude session.
 */
