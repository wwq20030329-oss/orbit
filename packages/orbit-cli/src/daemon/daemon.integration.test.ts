 ...
 *
 * This file boots one authenticated Orbit environment and restarts the daemon
...
const TERMINAL_STARTED_LABEL = 'orbit directly - likely launched from terminal';
...
async function stopAllTrackedSessions(): Promise<void> {
  ...
  await Promise.all(
    sessions.map((session: any) =>
      stopDaemonSession(session.orbitSessionId ?? `PID-${session.pid}`).catch(() => false)
    ),
  );
}

async function waitForTrackedSessionCount(
  ...
  await waitFor(async () => {
    latestSessions = await listDaemonSessions().catch(() => []);
    if (latestSessions.length !== expectedCount) {
      return false;
    }
    ...
    return expectedSessionIds.every((sessionId) =>
      latestSessions.some((session) => session.orbitSessionId === sessionId),
    );
  }, timeout, 250);
...
describe('Daemon Integration Tests', { timeout: 180_000 }, () => {
  ...
  beforeEach(async () => {
    ...
    void spawnOrbitCLI(['daemon', 'start'], {
      stdio: 'ignore',
    });
  ...
  it('should track session-started webhook from terminal session', async () => {
    ...
    expect(tracked.startedBy).toBe(TERMINAL_STARTED_LABEL);
    expect(tracked.orbitSessionId).toBe('test-session-123');
  ...
  it('should spawn & stop a session via HTTP (not testing RPC route, but similar enough)', async () => {
    ...
    const spawnedSession = sessions.find(
      (s: any) => s.orbitSessionId === response.sessionId
    );
    ...
    expect(spawnedSession.orbitSessionId).toBeDefined();
    await stopDaemonSession(spawnedSession.orbitSessionId);
  });
  ...
  it('stress test: spawn / stop', { timeout: 60_000 }, async () => {
    ...
    expect(results.every((result) => result.success && result.sessionId)).toBe(true);

    const sessions = await waitForTrackedSessionCount(sessionCount, sessionIds);
    ...
    const emptySessions = await waitForTrackedSessionCount(0);
  ...
  it('should track both daemon-spawned and terminal sessions', async () => {
    ...
    expect(terminalSession.startedBy).toBe(TERMINAL_STARTED_LABEL);
    ...
    await stopDaemonSession(daemonSession.orbitSessionId);
  ...
  it('should update session metadata when webhook is called', async () => {
    ...
    const sessions = await listDaemonSessions();
    const session = sessions.find((s: any) => s.orbitSessionId === spawnResponse.sessionId);
  ...
  it('should handle concurrent session operations', async () => {
    ...
    const daemonSessions = sessions.filter(
      (s: any) => s.startedBy === 'daemon' && spawnedSessionIds.includes(s.orbitSessionId)
    );
    expect(daemonSessions.length).toBeGreaterThanOrEqual(3);

    // Stop all spawned sessions
    for (const session of daemonSessions) {
      expect(session.orbitSessionId).toBeDefined();
      await stopDaemonSession(session.orbitSessionId);
    }
  });
   ...
   * 6. Daemon spawns new daemon via spawnOrbitCLI(['daemon', 'start'])
