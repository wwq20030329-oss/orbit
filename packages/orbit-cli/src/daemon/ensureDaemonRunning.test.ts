...
const mocks = vi.hoisted(() => ({
  ...
  mockIsDaemonRunningCurrentlyInstalledOrbitVersion: vi.fn(),
  mockCheckIfDaemonRunningAndCleanupStaleState: vi.fn(),
  mockSpawnOrbitCLI: vi.fn(),
...
vi.mock('./controlClient', () => ({
  isDaemonRunningCurrentlyInstalledOrbitVersion: mocks.mockIsDaemonRunningCurrentlyInstalledOrbitVersion,
  checkIfDaemonRunningAndCleanupStaleState: mocks.mockCheckIfDaemonRunningAndCleanupStaleState,
}))

vi.mock('@/utils/spawnOrbitCLI', () => ({
  spawnOrbitCLI: mocks.mockSpawnOrbitCLI,
}))
...
describe('ensureDaemonRunning', () => {
  beforeEach(() => {
    ...
    mocks.mockSpawnOrbitCLI.mockReturnValue({
      unref: vi.fn(),
    })
  ...
  it('returns without spawning when the daemon is already running', async () => {
    mocks.mockIsDaemonRunningCurrentlyInstalledOrbitVersion.mockResolvedValue(true)

    await ensureDaemonRunning()

    expect(mocks.mockSpawnOrbitCLI).not.toHaveBeenCalled()
  ...
  it('starts the daemon when the installed version is not running', async () => {
    ...
    mocks.mockIsDaemonRunningCurrentlyInstalledOrbitVersion.mockResolvedValue(false)
    ...
    expect(mocks.mockSpawnOrbitCLI).toHaveBeenCalledWith(['daemon', 'start-sync'], {
      detached: true,
      stdio: 'ignore',
      env: process.env,
    })
