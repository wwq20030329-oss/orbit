 ...
 * For instance we are not handling the case where we upgraded Orbit,
 ...
 * For instance when running `orbit daemon status` we can show more information.
 */
 ...
 */
export async function isDaemonRunningCurrentlyInstalledOrbitVersion(): Promise<boolean> {
  ...
  try {
    ...
    // we will get a new path or not when orbit is upgraded globally.
    ...
    /*
    const { spawnOrbitCLI } = await import('@/utils/spawnOrbitCLI');
    const orbitProcess = spawnOrbitCLI(['--version'], { stdio: 'pipe' });
    let version: string | null = null;
    orbitProcess.stdout?.on('data', (data) => {
      version = data.toString().trim();
    });
    await new Promise(resolve => orbitProcess.stdout?.on('close', resolve));
