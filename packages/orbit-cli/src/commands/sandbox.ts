...
export async function handleSandboxStatus(): Promise<void> {
    ...
    if (!config) {
        console.log('Sandbox is not configured. Run `orbit sandbox configure`.');
        return;
    }
...
export function handleSandboxHelp(): void {
    console.log(`
${chalk.bold('orbit sandbox')} - Sandbox management

${chalk.bold('Usage:')}
  orbit sandbox configure      Configure sandbox settings interactively
  orbit sandbox status         Show current sandbox configuration
  orbit sandbox disable        Disable sandboxing
  orbit sandbox help           Show this help
`);
}
