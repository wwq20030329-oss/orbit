...
function showAuthHelp(): void {
  console.log(`
${chalk.bold('orbit auth')} - Authentication management

${chalk.bold('Usage:')}
  orbit auth login [--force]    Authenticate with Orbit
  orbit auth logout             Remove authentication and machine data
  orbit auth status             Show authentication status
  orbit auth help               Show this help message
...
async function handleAuthLogout(): Promise<void> {
  ...
  const orbitDir = configuration.orbitHomeDir;
  ...
  console.log(chalk.blue('This will log you out of Orbit'));
  console.log(chalk.yellow('⚠️  You will need to re-authenticate to use Orbit again'));
  ...
  if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
    try {
      ...
      // Remove the local Orbit data directory (including legacy fallback data if configured).
      if (existsSync(orbitDir)) {
        rmSync(orbitDir, { recursive: true, force: true });
      }

      console.log(chalk.green('✓ Successfully logged out'));
      console.log(chalk.gray('  Run "orbit auth login" to authenticate again'));
    } catch (error) {
...
async function handleAuthStatus(): Promise<void> {
  ...
  if (!credentials) {
    ...
    console.log(chalk.gray('  Run "orbit auth login" to authenticate'));
    return;
  }
  ...
  if (settings?.machineId) {
    ...
    console.log(chalk.gray('  Run "orbit auth login --force" to fix this'));
  }

  // Data location
  console.log(chalk.gray(`\n  Data directory: ${configuration.orbitHomeDir}`));
