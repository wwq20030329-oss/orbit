/**
 * Minimal persistence functions for Orbit CLI
 * 
 * Handles settings and private key storage in ~/.orbit/ or local .orbit/
 */
...
interface Settings {
  ...
  daemonAutoStartWhenRunningOrbit?: boolean
  chromeMode?: boolean
  ...
  try {
    ...
    if (schemaVersion > SUPPORTED_SCHEMA_VERSION) {
      logger.warn(
        ...
        'Update Orbit CLI for full functionality.'
      );
    }
...
export async function writeSettings(settings: Settings): Promise<void> {
  if (!existsSync(configuration.orbitHomeDir)) {
    await mkdir(configuration.orbitHomeDir, { recursive: true })
  }
