 ...
 * for Orbit CLI including configuration, daemon status, logs, and links
 */
...
import { findAllOrbitProcesses } from '@/daemon/doctor'
...
export function getEnvironmentInfo(): Record<string, any> {
    return {
        ...
        orbitDir: configuration?.orbitHomeDir,
...
/**
 * Slim daemon status output for `orbit daemon status`
 */
export async function runDoctorDaemon(): Promise<void> {
    ...
    console.log(chalk.gray('\nRun `orbit doctor` for full diagnostics.\n'));
}
...
export async function runDoctorCommand(): Promise<void> {
    ...
    try {
        ...
        if (allProcesses.length > 0) {
            console.log(chalk.bold('🔍 All Orbit CLI Processes'));
            ...
            if (allProcesses.length > 1) {
                console.log(chalk.bold('\n💡 Process Management'));
                console.log(chalk.gray('To clean up runaway processes: orbit doctor clean'));
            }
        } else {
            console.log(chalk.red('❌ No Orbit CLI processes found'));
        }
    } catch (error) {
    ...
    const wrapperPath = join(projectRoot, 'bin', 'orbit.mjs');
    ...
    console.log(chalk.bold('\n🐛 Support & Bug Reports'));
    console.log(`Report issues: ${chalk.blue('https://github.com/wwq20030329-oss/orbit/issues')}`);
    console.log(`Documentation: ${chalk.blue('https://github.com/wwq20030329-oss/orbit/tree/main/docs')}`);

    // ── Concise useful info last (visible without scrolling) ──

    // Basic info
    console.log(chalk.bold('\n📋 Basic Information'));
    console.log(`Orbit CLI Version: ${chalk.green(packageJson.version)}`);
    console.log(`Platform: ${chalk.green(process.platform)} ${process.arch}`);
    console.log(`Node.js Version: ${chalk.green(process.version)}`);

    // Configuration
    console.log(chalk.bold('\n⚙️  Configuration'));
    console.log(`Orbit Home: ${chalk.blue(configuration.orbitHomeDir)}`);
