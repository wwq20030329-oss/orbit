...
import { type SessionEnvelope, type SessionTurnEndStatus } from '@orbit/wire';
import {
...
export class ApiSessionClient extends EventEmitter {
    ...
    private async flushOutbox() {
        ...
        while (this.pendingOutbox.length > 0) {
            ...
            const batchStartIndex = this.pendingOutbox.length - batchSize;
            const batch = this.pendingOutbox.slice(batchStartIndex);
            ...
            this.lastSeq = maxSeq;
            this.pendingOutbox.splice(batchStartIndex, batchSize);
        }
    }
    ...
    sendSessionEvent(event: {
        ...
        type: 'permission-mode-changed', mode: 'default' | 'acceptEdits' | 'bypassPermissions' | 'plan' | 'auto' | 'dontAsk'
    } | {
        type: 'ready'
    }, id?: string) {
        let content = {
