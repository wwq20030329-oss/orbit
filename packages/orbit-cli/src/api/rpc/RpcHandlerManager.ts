    ...
    async handleRequest(
        ...
        try {
            ...
            if (decryptedParams === null) {
                this.logger('[RPC] [ERROR] Failed to decrypt request params', { method: request.method });
                const errorResponse = { error: 'Failed to decrypt RPC request' };
            ...
            const rawResult = await handler(decryptedParams);
            const result = rawResult === undefined ? true : rawResult;
            this.logger('[RPC] Handler returned', {
                method: request.method,
                hasResult: rawResult !== undefined,
                normalizedUndefinedToTrue: rawResult === undefined,
            });
