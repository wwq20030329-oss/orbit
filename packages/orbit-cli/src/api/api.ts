...
export class ApiClient {
  ...
  async getOrCreateSession(opts: {
    ...
    timeoutMs?: number
    suppressNetworkFailure?: boolean
    ...
    if (this.credential.encryption.type === 'dataKey') {
      ...
      // const contentDataKey = await deriveKey(this.secret, 'Orbit EnCoder', ['content']);
    ...
    try {
      ...
      if (error && typeof error === 'object' && 'code' in error) {
        ...
        if (isNetworkError(errorCode)) {
          if (!opts.suppressNetworkFailure) {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response?.status) {
        ...
        if (status === 403 || status === 409) {
          ...
          console.log(chalk.yellow(
            `   → Run 'orbit doctor clean' to reset local state and generate a new machine ID`
          ));
          console.log(chalk.yellow(
