...
describe('Api server error handling', () => {
    ...
    describe('getOrCreateSession', () => {
        it('should return null when Orbit server is unreachable (ECONNREFUSED)', async () => {
        ...
        it('should return null when Orbit server cannot be found (ENOTFOUND)', async () => {
        ...
        it('should return null when Orbit server times out (ETIMEDOUT)', async () => {
            connectionState.reset();
            const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
        ...
        it('should return null when Orbit server request times out via axios (ECONNABORTED)', async () => {
            connectionState.reset();
            const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});

            mockPost.mockRejectedValue({ code: 'ECONNABORTED', message: 'timeout of 2500ms exceeded' });
