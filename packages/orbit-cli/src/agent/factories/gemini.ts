...
export interface GeminiBackendOptions extends AgentFactoryOptions {
  ...
  /** OAuth token from Orbit cloud (via 'orbit connect gemini') - highest priority */
  cloudToken?: string;
  ...
  /** Resume an existing Gemini local session if provided. */
...
export function createGeminiBackend(options: GeminiBackendOptions): GeminiBackendResult {
  ...
  // 1. Orbit cloud OAuth token (via 'orbit connect gemini') - highest priority
  ...
  let apiKey = options.cloudToken       // 1. Orbit cloud token (passed from runGemini)
  ...
  if (!apiKey) {
    logger.warn(`[Gemini] No API key found. Run 'orbit connect gemini' to authenticate via Google OAuth, or set ${GEMINI_API_KEY_ENV} environment variable.`);
  }
  ...
  if (options.resumeSessionId) {
    geminiArgs.push('--resume', options.resumeSessionId);
  }
  ...
  const backendOptions: AcpBackendOptions = {
    ...
    hasChangeTitleInstruction: (prompt: string) => {
      ...
      return lower.includes('change_title') ||
             ...
             lower.includes('mcp__orbit__change_title');
    },
  };
