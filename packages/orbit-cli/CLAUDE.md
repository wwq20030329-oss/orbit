# Orbit CLI Codebase Overview

## Project Overview

Orbit CLI (`orbit`) is a command-line tool that wraps Claude Code to enable remote control and session sharing. It's part of a three-component system:

1. **orbit-cli** (this project) - CLI wrapper for Claude Code and Codex
2. **orbit-app** - React Native / web client
3. **orbit-server** - Node.js server with Prisma (hosted at `https://api.cluster-fluster.com`)
...
- Private keys stored in `~/.orbit/access.key` with restricted permissions
...
## Starting the Daemon
```bash
# From the orbit-cli directory:
./bin/orbit.mjs daemon start

# With custom server URL (for local development):
ORBIT_SERVER_URL=http://localhost:3005 ./bin/orbit.mjs daemon start

# Stop the daemon:
./bin/orbit.mjs daemon stop

# Check daemon status:
./bin/orbit.mjs daemon status
```
