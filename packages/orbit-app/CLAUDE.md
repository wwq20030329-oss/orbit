...
- **TweetNaCl-compatible adapter** for end-to-end encryption across iOS, Android, and web
...
3. **Encryption**: End-to-end encryption using the shared NaCl-compatible adapter for all sensitive data
...
- Always try to use "useOrbitAction" from @sources/hooks/useOrbitAction.ts if you need to run some async operation, do not handle errors, etc - it is handled automatically.
