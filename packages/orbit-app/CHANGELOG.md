...
This preview release expands the current update with the latest Gemini models, a smarter voice onboarding flow, and more reliable Orbit CLI sessions for plan approvals and Codex turns.

- Update Orbit CLI with `npm i -g orbit`
...
- Fixed Orbit CLI plan approval flows so Accept and Reject buttons show up reliably in plan mode
- Fixed Orbit CLI background task updates and Codex turns that could sometimes hang or fail to complete
...
- Removed model configurations from agents. We were not able to keep up with the models so for now we are removing the configuration from the mobile app. You can still configure it through your CLIs, Orbit will simply use defaults.
...
This update introduces seamless GitHub integration, bringing your developer identity directly into Orbit while maintaining our commitment to privacy and security.
...
Welcome to Orbit - your secure, encrypted mobile companion for Claude Code. This inaugural release establishes the foundation for private, powerful AI interactions on the go.
