...
import type { MachineMetadata, NativeCliHistoryEntry, NativeCliTool } from './storageTypes';
...
export interface ResumeNativeCliHistoryOptions {
    machineId: string;
    tool: NativeCliTool;
    backendId: string;
...
export async function machineResumeNativeCliHistory(options: ResumeNativeCliHistoryOptions): Promise<SpawnSessionResult> {
    try {
        return await apiSocket.machineRPC<SpawnSessionResult, {
            tool: NativeCliTool;
            backendId: string;
            workingDirectory: string;
            title: string;
