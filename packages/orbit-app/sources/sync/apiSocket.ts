...
class ApiSocket {
    ...
    async sessionRPC<R, A>(sessionId: string, method: string, params: A): Promise<R> {
        ...
        if (result.ok) {
            const decrypted = await sessionEncryption.decryptRaw(result.result) as R | null;
            if (decrypted === null) {
                throw new Error('Failed to decrypt session RPC response');
            }
            return decrypted;
        }
    ...
    async machineRPC<R, A>(machineId: string, method: string, params: A): Promise<R> {
        ...
        if (result.ok) {
            const decrypted = await machineEncryption.decryptRaw(result.result) as R | null;
            if (decrypted === null) {
