...
export class Encryption {
    ...
    static async create(masterSecret: Uint8Array) {
        ...
        const contentDataKey = await deriveKey(masterSecret, 'Orbit EnCoder', ['content']);

        // Derive content data key keypair
        const contentKeyPair = sodium.crypto_box_seed_keypair(contentDataKey);

        // Derive anonymous ID
        const anonID = encodeHex((await deriveKey(masterSecret, 'Orbit Coder', ['analytics', 'id']))).slice(0, 16).toLowerCase();
    ...
    async decryptRaw(encrypted: string): Promise<any | null> {
        try {
            ...
            return decrypted[0] === undefined ? null : decrypted[0];
        } catch (error) {
            return null;
        }
    }
