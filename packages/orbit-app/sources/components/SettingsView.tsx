...
export const SettingsView = React.memo(function SettingsView() {
    ...
    const handleGitHub = async () => {
        const url = 'https://github.com/wwq20030329-oss/orbit';
    ...
    const handleReportIssue = async () => {
        const url = 'https://github.com/wwq20030329-oss/orbit/issues';
        const supported = await Linking.canOpenURL(url);
        if (supported) {
            await Linking.openURL(url);
        }
    ...
    // GitHub connection
    const [connectingGitHub, connectGitHub] = useOrbitAction(async () => {
        const params = await getGitHubOAuthParams(auth.credentials!);
        await Linking.openURL(params.url);
    });

    // GitHub disconnection
    const [disconnectingGitHub, handleDisconnectGitHub] = useOrbitAction(async () => {
    ...
    const [connectingAnthropic, connectAnthropic] = useOrbitAction(async () => {
        router.push('/settings/connect/claude');
    });

    // Anthropic disconnection
    const [disconnectingAnthropic, handleDisconnectAnthropic] = useOrbitAction(async () => {
        const confirmed = await Modal.confirm(
        ...
        <ItemList style={{ paddingTop: 0 }}>
            ...
            <View style={{ maxWidth: layout.maxWidth, alignSelf: 'center', width: '100%' }}>
                <View style={{ alignItems: 'center', paddingVertical: 24, backgroundColor: theme.colors.surface, marginTop: 16, borderRadius: 12, marginHorizontal: 16 }}>
                    {profile.firstName ? (
                        ...
                        // Logo view: App wordmark + version
                        <>
                            <BrandWordmark />
                        </>
                    )}
            ...
            <ItemGroup title={t('settings.about')} footer={t('settings.aboutFooter')}>
                ...
                <Item
                    ...
                    onPress={async () => {
                        const url = 'https://github.com/wwq20030329-oss/orbit/blob/main/PRIVACY.md';
                        const supported = await Linking.canOpenURL(url);
                        if (supported) {
                            await Linking.openURL(url);
                        }
                ...
                <Item
                    ...
                    onPress={async () => {
                        const url = 'https://github.com/wwq20030329-oss/orbit/blob/main/packages/orbit-app/TERMS.md';
                        const supported = await Linking.canOpenURL(url);
                        if (supported) {
                            await Linking.openURL(url);
                        }
