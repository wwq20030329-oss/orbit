...
function NewSessionScreen() {
    ...
    const pathItems = React.useMemo<PickerItem[]>(() => {
        ...
        return sessions
            .filter((value): value is Session => typeof value !== 'string')
    ...
    const preferredPathItem = React.useMemo(() => {
        const normalizedHomeDir = normalizePathForComparison(selectedHomeDir, selectedHomeDir);
        return pathItems.find((item) => normalizePathForComparison(item.key, selectedHomeDir) !== normalizedHomeDir)
            ?? pathItems[0]
            ?? null;
    }, [pathItems, selectedHomeDir]);

    // Default new sessions to the most recent project path, not the home directory.
    React.useEffect(() => {
        if (!selectedMachineId || !preferredPathItem) {
            return;
        }

        const normalizedSelectedPath = normalizePathForComparison(selectedPath, selectedHomeDir);
        const normalizedHomeDir = normalizePathForComparison(selectedHomeDir, selectedHomeDir);
        const shouldPromotePreferredPath = normalizedSelectedPath === null || normalizedSelectedPath === normalizedHomeDir;

        if (!shouldPromotePreferredPath) {
            return;
        }

        setSelectedPath(preferredPathItem.key);
    }, [selectedMachineId, preferredPathItem, selectedPath, selectedHomeDir, setSelectedPath]);
