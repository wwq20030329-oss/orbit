...
function getToneColor(tone: OrbitControlTone): string {
    if (tone === 'green') return '#30D158';
    if (tone === 'blue') return '#0A84FF';
    if (tone === 'orange') return '#FF9F0A';
    if (tone === 'red') return '#FF453A';
    return '#8E8E93';
}
        ...
        <ItemGroup title="Orbit Control">
            <View style={{ paddingHorizontal: 16, paddingBottom: 16, paddingTop: 8 }}>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginHorizontal: -6 }}>
                    {orbitTiles.map((tile) => {
                        const accent = getToneColor(tile.tone);
                        return (
                            <View
                                key={tile.key}
                                style={{
...
function SessionInfoContent({ session }: { session: Session }) {
    ...
    const handleCopySessionId = useCallback(async () => {
        ...
        try {
            ...
            Modal.alert(t('common.success'), t('sessionInfo.orbitSessionIdCopied'));
        } catch (error) {
    ...
    // Use OrbitAction for archiving - it handles errors automatically
    ...
    return (
        <>
            <ItemList>
                ...
                <OrbitControlPanel session={session} sessionStatus={sessionStatus} />

                {/* Session Details */}
                <ItemGroup>
                    <Item
                        title={t('sessionInfo.orbitSessionId')}
                ...
                {session.metadata && (
                    <ItemGroup title={t('sessionInfo.metadata')}>
                        ...
                        {session.metadata.orbitHomeDir && (
                            <Item
                                title={t('sessionInfo.orbitHome')}
                                subtitle={formatPathRelativeToHome(session.metadata.orbitHomeDir, session.metadata.homeDir)}
