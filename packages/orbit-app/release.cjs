...
function printAvailableOptions() {
  console.error("Available orbit-app release options:");
...
async function promptForAction() {
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    console.error("Interactive orbit-app release selection requires a TTY.");
  ...
  try {
    ...
    console.error(
      "Run `yarn install` from repository root, then run `yarn workspace orbit-app release` again."
    );
    process.exit(1);
  }

  const actionId = await select({
    message: "What should be released for orbit-app?",
    pageSize: 10,
...
async function main() {
  ...
  if (input && !action) {
    console.error(`Unknown orbit-app release option: ${input}`);
    printAvailableOptions();
    process.exit(1);
  }
  ...
  console.log(`Running orbit-app release option: ${action.label}`);
