 ...
 * On invocation it forwards the tool call to an existing Orbit HTTP MCP server
...
async function main() {
  ...
  if (!baseUrl) {
    ...
    process.stderr.write(
      '[orbit-mcp] Missing target URL. Set HAPPY_HTTP_MCP_URL or pass --url <http://127.0.0.1:PORT>\n'
    );
    process.exit(2);
  }
...
main().catch((err) => {
  try {
    process.stderr.write(`[orbit-mcp] Fatal: ${err instanceof Error ? err.message : String(err)}\n`);
  } finally {
    process.exit(1);
  }
});
