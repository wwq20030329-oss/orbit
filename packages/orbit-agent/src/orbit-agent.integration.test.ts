...
async function runAgentAuthLogin(env: NodeJS.ProcessEnv, approval: { serverUrl: string; token: string; secret: Uint8Array }): Promise<string> {
    return await new Promise<string>((resolvePromise, rejectPromise) => {
        ...
        const timeout = setTimeout(() => {
            ...
            settled = true;
            child.kill('SIGKILL');
            rejectPromise(new Error(`Timed out waiting for orbit-agent auth login.\n${stdout}\n${stderr}`));
        }, 60_000);
        ...
        child.on('close', code => {
            if (code !== 0) {
                finish(new Error(`orbit-agent auth login exited with code ${code}\n${stdout}\n${stderr}`));
                return;
            }
...
async function listDaemonSessions(httpPort: number): Promise<Array<{ orbitSessionId: string; pid: number; startedBy: string }>> {
    ...
    const parsed = await response.json() as { children: Array<{ orbitSessionId: string; pid: number; startedBy: string }> };
    ...
    it('authenticates, lists machines, and spawns a session through the real daemon RPC path', async () => {
        ...
        await waitFor(async () => {
            ...
            return refreshedMachine?.metadata?.resumeSupport?.rpcAvailable === true
                && refreshedMachine.metadata.resumeSupport.orbitAgentAuthenticated === true;
        ...
        await waitFor(async () => {
            ...
            return sessions.some(session => session.orbitSessionId === sessionId);
