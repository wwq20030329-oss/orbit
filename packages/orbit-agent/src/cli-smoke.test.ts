/**
 * CLI smoke tests for orbit-agent.
 *
...
const binPath = resolve(__dirname, '..', 'bin', 'orbit-agent.mjs');
...
describe('Smoke: CLI command surface', () => {
    ...
    describe('2. list command', () => {
        ...
        it('fails with auth error when not authenticated', () => {
            ...
            expect(stderr).toContain('orbit-agent auth login');
        });
    });
...
describe('Smoke: Error handling', () => {
    ...
    describe('server error handling (via API error mapper)', () => {
        it('HTTP 401 maps to re-authenticate message', async () => {
            ...
            const errorMsg = 'Authentication expired. Run `orbit-agent auth login` to re-authenticate.';
            expect(errorMsg).toContain('orbit-agent auth login');
        });
...
describe('Smoke: Interop — dataKey vs legacy encryption', () => {
    ...
    it('session created with legacy encryption (orbit-cli style) can be decrypted', () => {
        const creds = makeCredentials();
    ...
    it('messages encrypted with dataKey can be round-tripped', () => {
        const sessionKey = getRandomBytes(32);
        const messageContent = { role: 'user', content: { type: 'text', text: 'Hello from orbit-agent' } };
...
describe('Smoke: Full test suite runs', () => {
    ...
    it('config loads with correct defaults', () => {
        const origUrl = process.env.ORBIT_SERVER_URL;
        const origHome = process.env.ORBIT_HOME_DIR;
        ...
        try {
            ...
            expect(config.homeDir).toContain('.orbit');
            expect(config.credentialPath).toContain('agent.key');
        } finally {
            if (origUrl !== undefined) process.env.ORBIT_SERVER_URL = origUrl;
            if (origHome !== undefined) process.env.ORBIT_HOME_DIR = origHome;
        }
    });
});
