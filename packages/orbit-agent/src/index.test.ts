...
function runCli(...args: string[]): { stdout: string; stderr: string; exitCode: number } {
    try {
        const stdout = execFileSync(process.execPath, [
            ...
            binPath,
            ...args,
        ], { encoding: 'utf-8', env: { ...process.env, ORBIT_HOME_DIR: '/tmp/nonexistent-orbit-test' } });
...
describe('orbit-agent CLI', () => {
    it('should display help output', () => {
        const { stdout } = runCli('--help');
        expect(stdout).toContain('orbit-agent');
        expect(stdout).toContain('CLI client for controlling Orbit agents remotely');
    });
