...
function buildPublicAppRedirect(params: Record<string, string>) {
    const publicAppUrl =
...
export function connectRoutes(app: Fastify) {
    ...
    app.get('/v1/connect/github/callback', {
        ...
        if (!tokenData) {
            ...
            return reply.redirect(buildPublicAppRedirect({ error: 'invalid_state' }));
        }
        ...
        if (!clientId || !clientSecret) {
            return reply.redirect(buildPublicAppRedirect({ error: 'server_config' }));
        }
        ...
        try {
            ...
            if (tokenResponseData.error) {
                return reply.redirect(buildPublicAppRedirect({ error: tokenResponseData.error }));
            }
            ...
            if (!userResponse.ok) {
                return reply.redirect(buildPublicAppRedirect({ error: 'github_user_fetch_failed' }));
            }
            ...
            return reply.redirect(buildPublicAppRedirect({ github: 'connected', user: userData.login }));

        } catch (error) {
            log({ module: 'github-oauth' }, `Error in GitHub GET callback: ${error}`);
            return reply.redirect(buildPublicAppRedirect({ error: 'server_error' }));
        }
    });
