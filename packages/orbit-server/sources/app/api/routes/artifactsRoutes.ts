...
import { prismaBytes } from "@/utils/prismaBytes";
        ...
        try {
            ...
            const artifact = await db.artifact.create({
                data: {
                    id,
                    accountId: userId,
                    header: prismaBytes(privacyKit.decodeBase64(header)),
                    headerVersion: 1,
                    body: prismaBytes(privacyKit.decodeBase64(body)),
                    bodyVersion: 1,
                    dataEncryptionKey: prismaBytes(privacyKit.decodeBase64(dataEncryptionKey)),
                    seq: 0
                }
            });
    ...
    app.post('/v1/artifacts/:id', {
        ...
        try {
            ...
            if (header !== undefined && expectedHeaderVersion !== undefined) {
                updateData.header = prismaBytes(privacyKit.decodeBase64(header));
            ...
            if (body !== undefined && expectedBodyVersion !== undefined) {
                updateData.body = prismaBytes(privacyKit.decodeBase64(body));
