...
import { findMachineForAccount } from "@/app/data/machineStore";
import { findSessionForAccountById } from "@/app/data/sessionStore";
    ...
    socket.on('access-key-get', async (data: { sessionId: string; machineId: string }, callback: (response: any) => void) => {
        try {
            ...
            const [session, machine] = await Promise.all([
                findSessionForAccountById(userId, sessionId),
                findMachineForAccount(userId, machineId)
            ]);
