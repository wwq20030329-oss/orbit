...
Canonical wire specification package for Orbit clients and services.
...
`@orbit/wire` centralizes definitions for:
...
In this repository, consumer workspaces import `@orbit/wire` through package exports that point at `dist/*`.

That means on a clean checkout:
1. Build wire first: `yarn workspace @orbit/wire build`
...
yarn workspace @orbit/wire build
yarn workspace @orbit/wire test
```
